...
Husqy cares a lot about your privacy, thats why the `/privacy {subcommand}` command was created. The `/privacy {subcommand}` commands are powerful commands that give you the ability to: 1. Prevent Husqy from collecting data in the server, 2. View the amount of data references to you in the server that Husqy has stored, 3. Delete the data references to you in the server that Husqy has stored. The messages created by the `/privacy {subcommand}` commands are private to you and Husqy and can't be seen by other members despite being run in a server channel. Usage of this command will also NEVER be logged. Usage of the `/privacy {subcommand}` command does come with consequences to the use of Husqy services.
...
With the Husqy privacy commands it is possible to get the amount of references to your user Husqy has saved linked to the guild where the commands are executed. Just run the command `/privacy get_data` command. Doing this makes Husqy check your User ID, username, global name, display name and nickname. It returns the data in different embeds (split per module/function) which you can page through.
...
With the Husqy privacy commands it is also possible to remove the non-required references to your user Husqy has saved linked to the guild where the commands are executed. Just run the command `/privacy delete_data` command. Doing this navigates to a new view explaining the effects and two other buttons:

- `Cancel action`: As this option implies, cancels the actions and doesn't delete any data;
- `Delete my data`: Starts the deletion of your data. This action CAN NOT be reversed;

Required references are not deleted using this command. These references are needed for Husqy to provide you with some of its needed services but will not be saved longer than necessary. Leaving the server will delete the data linked to that server.

:::warning

- Some data is not removed, this includes but is not limited to, your data collection status and the link between you and a server if you are the owner of that server!
...
To change the status of your data collection settings, you can use the `/privacy info` command and press one of the following buttons:
...
Because the `/privacy {subcommand}` commands are per server, you must make changes in every server you share with Husqy if desired. To make it easier for you to get all servers you share with Husqy, you can run the `/privacy shared_servers` command. This will provide you with all server names and ID's for the servers you share with Husqy.

## Viewing server members data collection settings

A server administrator can login to the [Privacy Dashboard](#additional-information), this will show them an overview of which users have allowed and disallowed Husqy from collecting their data. Server administrators CAN NOT change the data collection status of other members apart from their own data collection status!
...
Husqy privacy provides the following commands:

- `/privacy get_data`;
- `/privacy delete_data`;
- `/privacy shared_servers`;
